...
public class ToDoListAdapter extends ArrayAdapter<ToDoItem> {
    ...
    private final Integer tab_index;
    
    public ToDoListAdapter(Activity context, List<ToDoItem> list,
            ToDoItemDataSource ds, Integer tab_index) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            viewHolder.image = (ImageView) view.findViewById(R.id.priorityImage);
                        ...
                        public void onCheckedChanged(CompoundButton buttonView,
                            ...
                            {	
                            	if (buttonView.isChecked()
                            			&& ((AllTasksActivity) context).getHide()) {
                            		List<ToDoItem> newList = ((AllTasksActivity) context)
                            				.getNewList();
                            ...
                            {
                            	if (buttonView.isChecked()
                                        && ((PendingTasksActivity) context).getHide()) {
                                    List<ToDoItem> newList = ((PendingTasksActivity) context)
                                            .getNewList();
                                    ToDoListAdapter.this.updateList(newList);
                                    ToDoListAdapter.this.notifyDataSetChanged();
                                }	
                            ...
                            {
                            	if (buttonView.isChecked()
                                        && ((CompletedTasksActivity) context).getHide()) {
                                    List<ToDoItem> newList = ((CompletedTasksActivity) context)
                                            .getNewList();
                                    ToDoListAdapter.this.updateList(newList);
                                    ToDoListAdapter.this.notifyDataSetChanged();
                                }
