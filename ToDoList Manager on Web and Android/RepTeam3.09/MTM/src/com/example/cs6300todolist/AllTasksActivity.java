...
public class AllTasksActivity extends Activity{
	...
	public List<ToDoItem> getNewList() {
		...
		if (hide) {
			newList = itemdatasource.getInCompleteListByUId(userKey);
		} else {
			newList = itemdatasource.getToDoListByUId(userKey);
		}
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		userName = getIntent().getStringExtra("com.example.cs6300todolist.username");
		Log.d("JI", "userKey inside alltasks is "+userKey);
		...
		Log.d("Ji","Inside All Tasks Activity -- >>>");
		this.setTitle("ToDo list for user " + userName);
		hide = false;
		sortType = SORT_DUEDATE;
		itemdatasource = new ToDoItemDataSource(this);
		itemdatasource.open();
		Log.d("JI", "Before ToDO ListAdapter");
		final List<ToDoItem> todoList = getNewList();
		/*    List<ToDoItem> newList = getNewList();
                final ListView toDoListView1 = (ListView) findViewById(R.id.all_taskListView);
                ToDoListAdapter adapter1 = (ToDoListAdapter) toDoListView1
                        .getAdapter();
                adapter1.updateList(newList);
                adapter1.notifyDataSetChanged(); */

		final ListView toDoListView = (ListView) findViewById(R.id.all_taskListView);
		...
		toDoListView.setOnItemLongClickListener(new OnItemLongClickListener() {
			...
			public boolean onItemLongClick(AdapterView<?> parent, View view,
				...
				PopupMenu popupMenu = new PopupMenu(AllTasksActivity.this, view);
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menu_priority:
			...
			final ListView toDoListViewp = (ListView) findViewById(R.id.all_taskListView);
	...
	public void onAddTaskClick(View view) {
		...
		// myIntent.putExtra("com.example.cs6300todolist.userid", userId);
		myIntent.putExtra("com.example.cs6300todolist.userkey", userKey);
		myIntent.putExtra("com.example.cs6300todolist.tab_index", 0);
		startActivityForResult(myIntent, 0);
	}
	...
	public void onSyncClick(View view) throws InterruptedException, ExecutionException, JSONException {
		...
		final List<ToDoItem> syncMobileList = getNewList();
