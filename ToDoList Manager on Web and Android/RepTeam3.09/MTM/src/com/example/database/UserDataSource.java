...
public class UserDataSource {
	  ...
	  public Users getUserByKey(String key) {
		    Cursor cursor = database.query(MySQLiteHelper.TABLE_USERS,
			        allColumns, MySQLiteHelper.COLUMN_KEY + " ='"+key+"'", null,
			        null, null, null);
		    cursor.moveToFirst();
			  if (cursor.isAfterLast())
				  return null;
	  ...
	  public int updateUsers(Users user) {
		  long id = user.getId();
		    ...
		    values.put(MySQLiteHelper.COLUMN_KEY, user.getEncodedKey());
		  
		  return database.update(MySQLiteHelper.TABLE_USERS, values, MySQLiteHelper.COLUMN_ID + " = " + id, null);

	  }
	  
	  public Users createUsers(String name, String pwd, String key) {
	    ContentValues values = new ContentValues();
	  ...
	  private Users cursorToUsers(Cursor cursor) {
		  ...
		  user.setEncodedKey(cursor.getString(3));
	    return user;
	  }
}
